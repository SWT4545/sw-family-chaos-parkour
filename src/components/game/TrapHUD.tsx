...
function CooldownRing({ pct, color }: { pct: number; color: string }) {
  const offset = CIRC * pct   // pct=0 → full ring, pct=1 → empty
  const ready  = pct === 0
  return (
...
function PlayerPanel({ player, state, side, keyHint }: PlayerPanelProps) {
  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-xl px-2 py-1.5 border backdrop-blur-sm
    ...
    >
      ...
      <div className="relative flex-shrink-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-base leading-none" style={{ filter: ready ? 'none' : 'grayscale(1)' }}>
            {state.trapIcon}
          </span>
        </div>
      </div>
      ...
      <div className={`min-w-0 ${isLeft ? '' : 'text-right'}`}>
        <p className="text-[9px] uppercase tracking-widest font-bold leading-none mb-0.5"
          style={{ color: colored }}>
          {state.trapName || '—'}
        </p>
      ...
      <AnimatePresence>
        {state.effectLabel && (
          <motion.span
            ...
            className="text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md flex-shrink-0"
...
export function TrapHUD({ player1, player2, chaosRef }: TrapHUDProps) {
  ...
  return (
    <div className="absolute inset-x-0 top-0 pointer-events-none select-none z-10"
      style={{ paddingTop: '52px' }}>  {/* sits below the GameHUD top bar */}
      <div className="flex items-start justify-between px-3 gap-2">
